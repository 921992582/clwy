...
router.post('/pay/:platform', userAuth, async function (req, res, next) {
  try {
    ...
    const bizContent = {
      product_code: productCode, out_trade_no: outTradeNo, subject: subject, total_amount: totalAmount
    };

    const url = alipaySdk.pageExecute(method, 'GET', {
      bizContent, returnUrl: process.env.ALIPAY_RETURN_URL, notify_url: process.env.ALIPAY_NOTIFY_URL,
    });
...
async function paidSuccess(outTradeNo, tradeNo, paidAt) {
  try {
    ...
    await sequelize.transaction(async (t) => {
      // 查询当前订单（在事务中）
      const order = await Order.findOne({
        where: {outTradeNo: outTradeNo}, transaction: t, lock: true,   // 增加共享锁
      });
...
async function getOrder(req) {
  ...
  const order = await Order.findOne({
    where: {
      outTradeNo: outTradeNo, userId: req.userId
    }
  });
